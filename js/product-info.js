...
function mostrarProducto(producto) {
    ...
    document.getElementById(
        "precio-unitario"
    ).textContent = `${moneda} ${precioUnitario.toLocaleString()}`;
    actualizarSubtotal();
            ...
            <div class="d-flex justify-content-between align-items-start mb-2">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div style="color: var(--primary-color);">${generarEstrellas(
                            comment.score
                        )}</div>
                    </div>
                </div>
...
function actualizarSubtotal() {
    ...
    document.getElementById(
        "subtotal"
    ).textContent = `${moneda} ${subtotal.toLocaleString()}`;
}

function agregarAlCarrito(producto) {
    // Suma cantidad si el producto ya existe
    ...
    if (indexExistente !== -1) {
        items[indexExistente].count =
            parseInt(items[indexExistente].count) + cantidad;
    } else {
        items.push({
            id: producto.id,
            name: producto.name,
